...
<div id="frame">
	...
	<div class="content">
		<div class="contact-profile" style="display: flex;overflow-x: auto;">
			<?php $sql2 = mysqli_query($conn,"SELECT * FROM `user_information`");
       while ($row = mysqli_fetch_assoc($sql2)) {
             if($row['last_login'] > time()){?>
               <div class="wrap">
                  <img src="http://localhost/project_one/img/<?php echo $row['profile_photo']; ?>" alt="" width="80" height="40"/>
      ...
      <?php  }else{ ?>
               ...
               <div class="wrap">
                  <img src="http://localhost/project_one/img/<?php echo $row['profile_photo']; ?>" alt=""width="80" height="40"/>
                  <span class="contact-status" style="background-color: lightgray;"></span>
               </div>
...
<?php     }  
      ...
      ?>
        ...
        <form action="" method="post">
          <input type="text" placeholder="Write your message..." id="inputbox" />
    			<input type="hidden" value="" class="hidenid" />
    			<i class="fa fa-paperclip attachment" aria-hidden="true"></i>
    			<button class="submit" type="submit" id="message_submit"><i class="fa fa-paper-plane" aria-hidden="true"></i></button>
        </form>
			</div>
		</div>
	</div>
...
<script >
  //$(".messages").animate({ scrollTop: $(".messages").innerHeight() }, "fast");

$(document).on('click','#contacts>ul>li',function(){
  var user_id = $(this).attr('data-for-tab');
  $(this).addClass('nowactive');
  $(this).siblings('li').removeClass('nowactive');
  $.ajax({
    url:"chat_message.php",
    type:"post",
    data:{user_id:user_id},
    success:function(data){
      $(".hidenid").attr('value',user_id);
      $(".messages").html(data);
    }
  });
});
...
$("#message_submit").click(function(e){
  ...
  var replie_message = $("#inputbox").val();
  var user_id = $(".hidenid").val();
...
$(document).ready(function(){
  ...
  $.ajax({
    ...
    success:function(data){
      $(".messages").html(data);
     //$(".messages").animate({ scrollTop: $(document).innerHeight() }, "fast");
        var a = $(document).height();
        var sc = $(".messages").scrollTop();
        //alert(sc);
    }
  });
}
